...
local __PUMPKIN_GRAD_SEQ = ColorSequence.new({
    ColorSequenceKeypoint.new(0,    __RGB(0, 255, 0)),
    ColorSequenceKeypoint.new(0.5,  __RGB(0, 200, 0)),
    ColorSequenceKeypoint.new(1,    __RGB(0, 150, 0))
})
...
-- Pumpkin colors (green)
local __PUMPKIN_NORMAL_COLOR = ColorSequence.new({
    ColorSequenceKeypoint.new(0,   __RGB(0, 255, 0)),
    ColorSequenceKeypoint.new(0.6, __RGB(0, 200, 0)),
    ColorSequenceKeypoint.new(1,   __RGB(0, 150, 0))
})

local __PUMPKIN_WAIT_COLOR = ColorSequence.new({
    ColorSequenceKeypoint.new(0,   __RGB(255, 0, 0)),
    ColorSequenceKeypoint.new(0.6, __RGB(200, 0, 0)),
    ColorSequenceKeypoint.new(1,   __RGB(150, 0, 0))
})
...
function BindableButtons.UpdateBButtonText(id, text, isWaiting, isGold, isPumpkin)
    ...
    if stroke then
        ...
        elseif isPumpkin then
            stroke.Color = isWaiting and __PUMPKIN_WAIT_COLOR or __PUMPKIN_NORMAL_COLOR
        else
...
-- ========== PUMPKIN JUMP+ SECTION ==========
local pumpkinSection = shared.AddSection("Pumpkin Jump+")

local pmpOnCooldown = false
local pumpkinJumpEnabled = false
local pmpDebounce = false
local autoGetPumpkin = false
local pmpJustRespawned = false
local pmpBigButtonSize = 200
...
RootMaid:GiveTask(PumpkinJumpMaid)

local function PMPResetCooldown()
    pmpOnCooldown = false
    local bigBtn = BBSystem.Buttons["pumpkinjump_big"]
    if bigBtn then bigBtn.Text = "Pumpkin Jump" end
    if pmpBindButton then
        BindableButtons.UpdateBButtonText("pumpkinjump_bind", "PJ", false, false, true)
    end
end

local function PMPStartCooldown()
    pmpOnCooldown = true
    pmpDebounce = false
    local bigBtn = BBSystem.Buttons["pumpkinjump_big"]
    if bigBtn then bigBtn.Text = "Wait" end
    if pmpBindButton then
        BindableButtons.UpdateBButtonText("pumpkinjump_bind", "Wait", true, false, true)
    end
    ...
    task.spawn(function()
        for i = 50, 1, -1 do  -- cooldown 50 seconds
            ...
            if bigBtn then bigBtn.Text = tostring(i) end
            if pmpBindButton then
                BindableButtons.UpdateBButtonText("pumpkinjump_bind", tostring(i), true, false, true)
            end
            task.wait(1)
        end
        if pmpOnCooldown then PMPResetCooldown() end
    end)
end

local function PMPGetCenterPosition()
    local character = LocalPlayer.Character
    if character and character:FindFirstChild("HumanoidRootPart") then
        local camera = Services.Workspace.CurrentCamera
        local lookDir = camera.CFrame.LookVector
...
local function PMPMakeCharacterJump()
    local character = LocalPlayer.Character
    if character then
        local humanoid = character:FindFirstChild("Humanoid")
        if humanoid then
...
local function UnequipPumpkin()
    task.spawn(function()
        local character = LocalPlayer.Character
        if character then
            local bomb = character:FindFirstChild(PUMPKIN_NAME)
            if bomb then
                bomb.Parent = LocalPlayer.Backpack or character
            end
        end
...
local function GetAnyPumpkin()
    ...
    local backpack = LocalPlayer:FindFirstChild("Backpack")
    if backpack then
        bomb = backpack:FindFirstChild(PUMPKIN_NAME)
        if bomb then
            bomb.Parent = character
            task.wait(0.05)
            return true, bomb
    ...
    local success = pcall(function()
        Services.ReplicatedStorage.Remotes.Extras.ReplicateToy:InvokeServer("Pumpkin2025")
    end)

    if success then
        for _ = 1, 5 do
            bomb = character:FindFirstChild(PUMPKIN_NAME)
            if bomb then return true, bomb end
...
local function FastPumpkinJump()
    ...
    if success and bomb then
        local position = PMPGetCenterPosition()
        if position then
            local remote = bomb:FindFirstChild("Remote")
            if remote then
                PlaySound(Sounds.Click)
...
local function IsHoldingPumpkin()
    local character = LocalPlayer.Character
    if not character then return false end
    return character:FindFirstChild(PUMPKIN_NAME) ~= nil
end

local pmpActiveTouches = {}

PumpkinJumpMaid:GiveTasks(
    Services.UserInputService.InputBegan:Connect(function(input, gp)
        if gp then return end
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            pmpActiveTouches[input] = {startPosition = input.Position, startTime = tick(), moved = false}
        end
    end),
    Services.UserInputService.InputChanged:Connect(function(input)
    ...
    Services.UserInputService.InputEnded:Connect(function(input, gp)
        ...
        pmpActiveTouches[input] = nil
    end),
    LocalPlayer.CharacterAdded:Connect(function()
        PMPResetCooldown()
        pmpActiveTouches = {}
        pmpJustRespawned = true
        task.wait(1)
        pmpJustRespawned = false
        if autoGetPumpkin then
            task.wait(0.2)
...
pumpkinSection:AddLabel("Pumpkin Jump Options")
pumpkinSection:AddToggle("Enable Auto Pumpkin Jump", function(bool) pumpkinJumpEnabled = bool end)

pumpkinSection:AddToggle("Auto-Get Pumpkin", function(bool)
    autoGetPumpkin = bool
    if bool then
        pcall(function() Services.ReplicatedStorage.Remotes.Extras.ReplicateToy:InvokeServer("Pumpkin2025") end)
        task.wait(0.1)
...
pumpkinSection:AddToggle("Enable PJ Big Button", function(e)
    if e then
        ...
        if btn then
            ...
            btn.Size = __UD2(0, pmpBigButtonSize, 0, pmpBigButtonSize * 0.375)
        end
    else
        DeleteBigButton("pumpkinjump_big")
    end
end)

pumpkinSection:AddSlider("PJ Big Button Size", 50, 300, 200, function(value)
    pmpBigButtonSize = value
    local btn = BBSystem.Buttons["pumpkinjump_big"]
    if btn then
        btn.Size = __UD2(0, pmpBigButtonSize, 0, pmpBigButtonSize * 0.375)
    end
end)

pumpkinSection:AddToggle("Enable PJ Bind Button", function(e)
    if e then
        BindableButtons.AddBButton("pumpkinjump_bind", "PJ", FastPumpkinJump, false)
        pmpBindButton = BindableButtons.Buttons["pumpkinjump_bind"]
        if pmpBindButton then
            local screen = Services.Workspace.CurrentCamera.ViewportSize
            ...
            local ripple = pmpBindButton:FindFirstChild("@ripple")
            if ripple then
                ripple.BackgroundColor3 = __RGB(0, 255, 0)
            end
            BindableButtons.UpdateBButtonText("pumpkinjump_bind", pmpOnCooldown and "Wait" or "PJ", pmpOnCooldown, false, true)
        end
    else
        BindableButtons.DeleteBButton("pumpkinjump_bind")
        pmpBindButton = nil
    end
end)

pumpkinSection:AddSlider("PJ Bind Button Size", 5, 25, 11, function(value)
    pmpBindButtonSize = value / 100
    if pmpBindButton then
        local screen = Services.Workspace.CurrentCamera.ViewportSize
        pmpBindButton.Size = __UD2(pmpBindButtonSize * (screen.Y / screen.X), 0, pmpBindButtonSize, 0)
    end
